...
class EasyTransformer(HookedRootModule):
    ...
    def forward(
        ...
        # vocab_mean_pos: Optional[List[int]] = None,
        pos_offset=None,
        ...
        # for batch_idx in range(input.shape[0] // batch_size):
        ...
        # print(pos_offset)
        # if vocab_mean_pos is not None:
        #     embed = self.embed(tokens)
        #     embed[:, vocab_mean_pos] = self.embed.W_E.data.mean(dim=0)
        ...
        for i, block in enumerate(self.blocks):
            ...
            device_id = int(i / self.cfg.n_layers * gpu_num)
            block = block.to(f'cuda:{device_id}')
            residual = residual.to(f'cuda:{device_id}')
        ...
        **model_kwargs,
        ...
        if hf_model is None:
            if checkpoint is not None:
                ...
                # print('aaaa')
                # parallelize(hf_model, num_gpus=2, fp16=True, verbose='detail')
